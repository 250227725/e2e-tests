...
5. После подтверждения, но до записи: вычисли новый хэш (`document_authoring.py hash --kind tc --id TC-<CODE>-<NNN>`), сравни с хэшем в существующем файле теста (если файла нет — пропусти этот шаг). При любом расхождении удали файл теста и `TC-<...>.selectors.json` целиком — по `playwright-implementation.md`, раздел «Редактирование AA и TC» → «`/edit-tc`».
